...
const googleLogin = async (req, res) => {
  ...
  try {
    ...
    db.run(
      "INSERT INTO users (id, name, email, password, googleSub) VALUES (?, ?, ?, ?, ?) ON CONFLICT(email) DO UPDATE SET name = excluded.name, googleSub = excluded.googleSub",
      [id, name, email, password, googleSub],
      function (err) {
        if (err) {
          return res.status(500).json({
